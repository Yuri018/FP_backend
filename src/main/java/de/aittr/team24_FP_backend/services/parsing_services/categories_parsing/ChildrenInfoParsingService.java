...
public class ChildrenInfoParsingService extends CommonParsingTextService {
    ...
    public void childrenInfoHamburgParsingSaveService() {
        try {
            List<String> localLinkList = getDocumentToLocalInfoPages(INFO_URL, REFERRER, 3, 38);
            String urlToHamburgPage = localLinkList.get(0);
            String urlToHamburgOtherLink = localLinkList.get(1);

            Document docToOtherLocalCategory = getDocument(urlToHamburgOtherLink, urlToHamburgPage);
            Element elCategories = docToOtherLocalCategory.getElementsByClass("root-cat").get(5);
            urlToHamburgOtherLink = urlToHamburgOtherLink.substring(0, urlToHamburgOtherLink.length() - 1);
            String urlToHomeChildren = urlToHamburgOtherLink + elCategories.child(0).attr("href");
            Document docHomeChildren = getDocument(urlToHomeChildren, urlToHamburgOtherLink);
            String urlToKindergarten = urlToHamburgPage + docHomeChildren.getElementsByClass("subcategory").first().child(0).attr("href");
            Document docKindergarten = getDocument(urlToKindergarten, urlToHomeChildren);
            Elements elsKindergarten = docKindergarten.getElementsByClass("catalog-item catalog-item-D");

            City city = cityRepository.findByName("HAMBURG");

            for (int i = 0; i < elsKindergarten.size(); i++) {
                ChildrenInfo childrenInfo = new ChildrenInfo();

                String title = elsKindergarten.get(i).getElementsByClass("title").text();
                String tel = elsKindergarten.get(i).getElementsByClass("phone").text();
                String description = elsKindergarten.get(i).getElementsByClass("description").text();
                String address = elsKindergarten.get(i).getElementsByClass("address").text();
    ...
    public void childrenInfoFrankfurtParsingSaveService() {

        try {
            List<String> localLinkList = getDocumentToLocalInfoPages(INFO_URL, REFERRER, 5, 38);
            String urlToFrankfurtPage = localLinkList.get(0);
            String urlToFrankfurtOtherLink = localLinkList.get(1);

            Document docToOtherLocalCategory = getDocument(urlToFrankfurtOtherLink, urlToFrankfurtPage);
            Element elCategories = docToOtherLocalCategory.getElementsByClass("root-cat").get(5);
            urlToFrankfurtOtherLink = urlToFrankfurtOtherLink.substring(0, urlToFrankfurtOtherLink.length() - 1);
            String urlToHomeChildren = urlToFrankfurtOtherLink + elCategories.child(0).attr("href");
            Document docHomeChildren = getDocument(urlToHomeChildren, urlToFrankfurtOtherLink);
            String urlToKindergarten = urlToFrankfurtPage + docHomeChildren.getElementsByClass("subcategory").first().child(0).attr("href");
            Document docKindergarten = getDocument(urlToKindergarten, urlToHomeChildren);
            Elements elsKindergarten = docKindergarten.getElementsByClass("catalog-item catalog-item-D");

            City city = cityRepository.findByName("FRANKFURT");
...
//
//        try {
//            List<String> localLinkList = getDocToLocalPages(URL, REFERRER, 5, 38);
//            String urlToFrankfurtPage = localLinkList.get(0);
//            String urlToFrankfurtOtherLink = localLinkList.get(1);
//
//            Document docToOtherLocalCategory = getDoc(urlToFrankfurtOtherLink, urlToFrankfurtPage);
//            Element elCategories = docToOtherLocalCategory.getElementsByClass("root-cat").get(5);
//            urlToFrankfurtOtherLink = urlToFrankfurtOtherLink.substring(0, urlToFrankfurtOtherLink.length() - 1);
//            String urlToHomeChildren = urlToFrankfurtOtherLink + elCategories.child(0).attr("href");
//            Document docHomeChildren = getDoc(urlToHomeChildren, urlToFrankfurtOtherLink);
////            String urlToChildOccasions = urlToFrankfurtPage + docHomeChildren.getElementsByClass("subcategory").first().child(6).attr("href");
...
//            String urlToKindergarten = urlToFrankfurtPage + docHomeChildren.getElementsByClass("subcategory").first().child(0).attr("href");
//            Document docKindergarten = getDoc(urlToKindergarten, urlToHomeChildren);
//            Elements elsKindergarten = docKindergarten.getElementsByClass("catalog-item catalog-item-D");
//            String title = null;
//            String tel = null;
...
//            for (int i = 0; i < elsKindergarten.size(); i++) {
//                title = elsKindergarten.get(i).getElementsByClass("title").text();
//                tel = elsKindergarten.get(i).getElementsByClass("phone").text();
//                description = elsKindergarten.get(i).getElementsByClass("description").text();
//                address = elsKindergarten.get(i).getElementsByClass("address").text();
////                urlToHomePage = elsKindergarten.get(i).select("div.www > a").first().text();
...
//
//
////            System.out.println(urlToFrankfurtPage);
////            System.out.println(urlToFrankfurtOtherLink);
////            System.out.println(urlToHomeChildren);
////            System.out.println(urlToKindergarten);
//
//        } catch (IOException e) {
//            e.printStackTrace();
//        }
